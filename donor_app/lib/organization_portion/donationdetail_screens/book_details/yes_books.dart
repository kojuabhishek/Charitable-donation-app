...
class YesBooks extends StatefulWidget {
  //String lati, longi;
  const YesBooks({Key? key}) : super(key: key);

  @override
  _YesBooksState createState() => _YesBooksState();
}

class _YesBooksState extends State<YesBooks> {
  ...
  Future<void> getData() async {
    ...
    QuerySnapshot snapshot = await inst.collection("Book Location").get();
  ...
  double haversine(double lat1, double lon1, e, int i) {
    double dLat = (double.parse(e[i]['latitude']).abs() - lat1) * pi / 180;
    double dLon = (double.parse(e[i]['longitude']).abs() - lon1) * pi / 180;
    lat1 = lat1 * pi / 180;
    double lon2 = double.parse(e[i]['longitude']).abs() * pi / 180;
    double a =
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          widgets_list['data']![index]["category"],
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          widgets_list['data']![index]["language"],
                          style: TextStyle(
                            fontSize: 18,
                          ),
                        ),
                          ...
                          Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) => GoogleMapScreen(
                                  category: widgets_list['data']![index]
                                      ["category"],
                                  email: widgets_list['data']![index]["email"],
                                  f_name: widgets_list['data']![index]
                                      ["first name"],
                                  language: widgets_list['data']![index]
                                      ["language"],
                                  numb: widgets_list['data']![index]["num"],
