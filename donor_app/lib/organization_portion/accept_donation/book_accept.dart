...
import 'package:donor_app/organization_portion/send%20email/booksend_mail.dart';
...
class BookAccept extends StatefulWidget {
  String f_name,
      s_name,
      email,
      category,
  ...
  @override
  _BookAcceptState createState() => _BookAcceptState();
}

class _BookAcceptState extends State<BookAccept> {
...
//      ..bccRecipients.add(Address('bccAddress@example.com'))
      ..subject = 'Book Donation Accepted ${DateTime.now()}'
      ..text = 'This is to notify the book donation is accepted..\n'
      ..html =
          "<h1>Your donation of book is accepted by ${loggedInOrg.firstName} ${loggedInOrg.secondName}</h1>\n<h2>Keep on helping.We will contact you soon.\nContact us:\n${loggedInOrg.email}\nPhone:${loggedInOrg.phNumber}</h2>";
            ...
            ElevatedButton(
                onPressed: () async {
                  ...
                  Fluttertoast.showToast(
                      msg: "written and moved to Confirm book");

                  var collection =
                      FirebaseFirestore.instance.collection('Book Location');
                  ...
                  Navigator.of(context).pushReplacement(
                      MaterialPageRoute(builder: (context) => Org_Books()));
                },
                child: Text("Confirm")),
          ],
        )),
