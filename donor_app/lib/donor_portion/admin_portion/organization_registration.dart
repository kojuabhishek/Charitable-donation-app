...
class OrgRegistrationScreen extends StatefulWidget {
  String lat, lng;
  OrgRegistrationScreen({Key? key, required this.lat, required this.lng})
      : super(key: key);

  @override
  _OrgRegistrationScreenState createState() => _OrgRegistrationScreenState();
}
...
class _OrgRegistrationScreenState extends State<OrgRegistrationScreen> {
  ...
  final phNumberEditingController = new TextEditingController();
  ...
  Widget build(BuildContext context) {
    ...
    // phonenumber field
    final phNumberField = TextFormField(
        autofocus: false,
        controller: phNumberEditingController,
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        leading: IconButton(
          icon: Icon(Icons.arrow_back, color: Color(0xff4267B2)),
          onPressed: () {
  ...
  postDetailsToFirestore() async {
    ...
    userModel.phNumber = phNumberEditingController.text;

    userModel.role = 'NGO';
    userModel.lat = widget.lat;
    userModel.lon = widget.lng;

    await firebaseFirestore
        .collection("organization")
        .doc(user.uid)
        .set(userModel.toMap());
    Fluttertoast.showToast(msg: "Organization Created");
